...
let paragraph = document.querySelector(".blog-description");
let maxLength = 115;
...
$.ajax(settings).done(function (response) {
  ...
  response.data.forEach(function (blog) {
    console.log("kaia");
    if (blog.publish_date > today) {
    } else {
      let newDiv = $("<div>").attr("id", "div_" + blog.id).addClass("blog-content-div");
      ...
      let title = $('<p>').text(blog.description).addClass("blog-title");
